component accessors="true" hint="A CFML binding for Selenium 1.0 (a.k.a. 'RC' -- predecessor of the now-preferred 'WebDriver')" {
	...
	*/
	...
	public any function init (string host = "localhost", numeric port = 4444,
		numeric executionDelay = 200,
		string seleniumJarPath = "../../lib/selenium-server-standalone-2.53.0.jar",
		boolean verbose = false,
		string seleniumServerArguments = "",
	...
	public void function waitForElementPresent(required string locator, numeric timeout = variables.waitTimeout) {
		...
		while (not isElementPresent(arguments.locator)) {
			...
			if (getTickCount() gte endTickCount) {
				throw (type="elementNotFound", message="The element: #arguments.locator# was not found after #arguments.timeout/1000# seconds.");
			}
		}
	}
	...
	public void function waitForElementVisible(required string locator, numeric timeout = variables.waitTimeout) {
		...
		while (not isVisible(arguments.locator)) {
			sleep(100);
			if (getTickCount() gte endTickCount) {
				throw (type="elementNotVisible", message="The element: #arguments.locator# was not visible after #arguments.timeout/1000# seconds.");
			}
		}
	}
	...
	public void function waitForElementNotVisible(required string locator, numeric timeout = variables.waitTimeout) {
		...
		while (isVisible(arguments.locator)) {
			sleep(100);
			if (getTickCount() gte endTickCount) {
				throw (type="elementStillVisible", message="The element: #arguments.locator# was still visible after #arguments.timeout/1000# seconds.");
			}
		}
	}
